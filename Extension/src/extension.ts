...
export function activate(context: vscode.ExtensionContext) {
    ...
    console.log('Congratulations, your extension "flocklive" is now active!');

    let todofyLive = new TodofyLive();
    let config = new Config();
    
    let disposable = vscode.commands.registerCommand('extension.flocklive', () => {
        var editor, data:dataToBot, selection, text:string, regex, result;

        // configuration should be set
        if (!config.Load()) {
            vscode.window.showInformationMessage('Please configure flocklive');
            return;            
        }
        ...
        if (typeof result[1] != 'undefined' && typeof result[2] != 'undefined') {
            ...
            // get 3 lines from top, 3 lines from bottom and send it to flock
...
class Config {
    ...
    public Load() {
        var config = vscode.workspace.getConfiguration('flocklive.flockBot');
